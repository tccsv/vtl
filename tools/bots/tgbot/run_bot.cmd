...
rem Корень репозитория = на три уровня выше tools\bots\tgbot\
cd /d "%~dp0..\..\.."
...
if not exist "%SECRETS%" (
  ...
  echo     copy tools\bots\tgbot\secrets.env.example tools\bots\tgbot\secrets.env
  pause & exit /b 1
)
...
if not exist "app\main_firyulin.exe" (
  ...
  echo Собери сначала:  tools\bots\tgbot\build_bot.cmd
  pause & exit /b 1
)
