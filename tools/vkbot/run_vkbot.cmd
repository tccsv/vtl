...
rem Корень репозитория = на два уровня выше tools\vkbot\
cd /d "%~dp0..\.."
...
if not exist "%SECRETS%" (
  ...
  echo     copy tools\vkbot\secrets.env.example tools\vkbot\secrets.env
  pause & exit /b 1
)
...
if not exist "app\main_schulgin.exe" (
  ...
  echo Собери сначала:  tools\vkbot\build_vkbot.cmd
  pause & exit /b 1
)
