...
#define VTL_HTML_INITIAL_CAPACITY 64
...
VTL_AppResult VTL_html_MarkerListReserve(VTL_html_MarkerList* list, size_t capacity)
{
    ...
    VTL_html_Marker* new_items = (VTL_html_Marker*)realloc(
        list->items, capacity * sizeof(VTL_html_Marker));
    if (!new_items) {
                                      ...
                                      VTL_html_Marker marker)
{
    ...
    if (list->length >= list->capacity) {
        size_t new_cap = list->capacity ? list->capacity * 2 : VTL_HTML_INITIAL_CAPACITY;
        VTL_AppResult res = VTL_html_MarkerListReserve(list, new_cap);
        if (res != VTL_res_kOk) {
            return res;
        }
    }
