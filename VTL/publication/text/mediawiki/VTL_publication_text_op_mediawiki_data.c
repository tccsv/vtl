#include <VTL/publication/text/mediawiki/VTL_publication_text_op_mediawiki_data.h>

#include <stdlib.h>
#include <stdint.h>


#define VTL_MEDIAWIKI_INITIAL_CAPACITY 64


VTL_AppResult VTL_mediawiki_MarkerListInit(VTL_mediawiki_MarkerList* list)
{
    if (!list) return VTL_res_kInvalidParamErr;
    list->items = NULL;
    list->length = 0;
    list->capacity = 0;
    return VTL_res_kOk;
}

VTL_AppResult VTL_mediawiki_MarkerListReserve(VTL_mediawiki_MarkerList* list, size_t capacity)
{
    if (!list) return VTL_res_kInvalidParamErr;
    if (capacity <= list->capacity) return VTL_res_kOk;
    if (capacity > SIZE_MAX / sizeof(VTL_mediawiki_Marker)) return VTL_res_kMemAllocErr;
    VTL_mediawiki_Marker* new_items = (VTL_mediawiki_Marker*)realloc(
        list->items, capacity * sizeof(VTL_mediawiki_Marker));
    if (!new_items) return VTL_res_kMemAllocErr;
    list->items = new_items;
    list->capacity = capacity;
    return VTL_res_kOk;
}

VTL_AppResult VTL_mediawiki_MarkerListPush(VTL_mediawiki_MarkerList* list,
                                            VTL_mediawiki_Marker marker)
{
    if (!list) return VTL_res_kInvalidParamErr;
    if (list->length >= list->capacity) {
        size_t new_cap = list->capacity ? list->capacity * 2 : VTL_MEDIAWIKI_INITIAL_CAPACITY;
        VTL_AppResult res = VTL_mediawiki_MarkerListReserve(list, new_cap);
        if (res != VTL_res_kOk) return res;
    }
    list->items[list->length++] = marker;
    return VTL_res_kOk;
}

void VTL_mediawiki_MarkerListClear(VTL_mediawiki_MarkerList* list)
{
    if (list) list->length = 0;
}

void VTL_mediawiki_MarkerListFree(VTL_mediawiki_MarkerList* list)
{
    if (!list) return;
    free(list->items);
    list->items = NULL;
    list->length = 0;
    list->capacity = 0;
}
