                                                    ...
                                                    const VTL_publication_marked_text_Configs* p_configs)
{
    ...
    app_result = VTL_publication_marked_text_CheckAndGenStandartMDFile(p_marked_text, src_file_name, src_markup_type, 
                                                        p_configs->files[VTL_PUBLICATION_TEXT_TYPE_STANDART_MD_INDEX],
                                                        p_configs->flags);
    ...
    app_result = VTL_publication_marked_text_CheckAndGenTelegramMDFile(p_marked_text, src_file_name, src_markup_type, 
                                                        p_configs->files[VTL_PUBLICATION_TEXT_TYPE_TELEGRAM_MD_INDEX],
                                                        p_configs->flags);
    if(app_result != VTL_res_kOk)
    {
        return app_result;
    ...
    app_result = VTL_publication_marked_text_CheckAndGenBBFile(p_marked_text, src_file_name, src_markup_type, 
                                                        p_configs->files[VTL_PUBLICATION_TEXT_TYPE_BB_INDEX], 
                                                        p_configs->flags);
    if(app_result != VTL_res_kOk)
    {
        return app_result;
    ...
    app_result = VTL_publication_marked_text_CheckAndGenHTMLFile(p_marked_text, src_file_name, src_markup_type, 
                                                        p_configs->files[VTL_PUBLICATION_TEXT_TYPE_HTML_INDEX], 
                                                        p_configs->flags);
    if(app_result != VTL_res_kOk)
    {
        return app_result;
    ...
    app_result = VTL_publication_marked_text_CheckAndGenRegularText(p_marked_text, src_file_name, src_markup_type, 
                                                        p_configs->files[VTL_PUBLICATION_TEXT_TYPE_REGULAR_INDEX], 
                                                        p_configs->flags);
    if(app_result != VTL_res_kOk)
    {
        return app_result;
...
VTL_publication_app_result VTL_publication_text_gen_Init(VTL_publication_marked_text* p_marked_text) {
    ...
    p_marked_text->parts = (VTL_publication_marked_text_part*)malloc(sizeof(VTL_publication_marked_text_part));
    if (!p_marked_text->parts) {
        return VTL_publication_res_kError;
    }

    p_marked_text->length = 0;
    return VTL_publication_res_kOk;
}

void VTL_publication_text_gen_Free(VTL_publication_marked_text* p_marked_text) {
    if (p_marked_text && p_marked_text->parts) {
        free(p_marked_text->parts);
        p_marked_text->parts = NULL;
        p_marked_text->length = 0;
    }
}
