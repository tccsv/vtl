/*
 * Smoke-тест + бенчмарк для telegram-модуля.
 * Самодостаточный — собирается через VTL/publication/text/telegram/CMakeLists.txt
...
static int run_case(const char* name, const char* input)
{
    ...
    if (VTL_telegram_ParseTextSequential(&src, &a) != VTL_res_kOk || !a) {
        printf("[%s] FAIL: sequential parse\n", name);
        return 1;
    }
    if (VTL_telegram_ParseTextParallel(&src, &b) != VTL_res_kOk || !b) {
...
/* сгенерировать большой Telegram MD-текст в памяти, конкатенируя fragment.
...
int main(void)
{
    ...
    printf("=== Telegram MarkdownV2 smoke tests ===\n\n");

    int fails = 0;
    fails += run_case("simple-bold",   "Hello *world*!");
    fails += run_case("simple-italic", "This is _italic_ text");
    fails += run_case("simple-strike", "Crossed ~out~ word");
    fails += run_case("all-three",     "Mix of *bold*, _italic_, and ~strike~");
    fails += run_case("snake-case",    "snake_case_var should stay plain");
    fails += run_case("escaped-star",  "Escaped \\*not bold\\* here");
    fails += run_case("nested",        "*bold _bold-italic_ still-bold*");
    ...
    /* инвариант на большом тексте: Sequential ≡ Parallel */
    {
        ...
        if (big) {
            ...
            VTL_telegram_ParseTextSequential(&src, &s);
            VTL_telegram_ParseTextParallel(&src, &p);
