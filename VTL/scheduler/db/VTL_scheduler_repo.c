...
/* Преобразовать строку "TG" / "REDDIT" / "VK" → enum               */
...
/* unix timestamp → ISO-8601 строка "YYYY-MM-DD HH:MM:SS" (UTC)       */
...
/* ISO-8601 строка → unix timestamp (UTC). Использует strptime.        */
static time_t iso_to_ts(const char *s) {
    if (!s || *s == '\0') return 0;
    struct tm tm_val;
    memset(&tm_val, 0, sizeof(tm_val));
#ifdef _WIN32
    /* MinGW не всегда имеет strptime; используем sscanf */
...
/* Скопировать строку из PQgetvalue (возвращает NULL если поле NULL)   */
...
/* Заполнить VTL_scheduler_Post из строки PGresult                    */
...
VTL_AppResult VTL_scheduler_repo_Open(VTL_scheduler_Repo *repo,
    ...
    /* Используем существующий VTL_connect_to_db                       */
...
VTL_AppResult VTL_scheduler_repo_GetPending(VTL_scheduler_Repo *repo,
    ...
    /* Выбираем все неотработанные записи, время которых наступит
     * в течение следующих lookahead_sec секунд (или уже прошло).      */
    const char *sql =
            "SELECT id, to_char(send_date_time,'YYYY-MM-DD HH24:MI:SS'),"
            "       created_user, content_type, content,"
            "       sn_type, metadata, executed"
