...
{
    VTL_tgbot_sessionTable t;
    VTL_tgbot_session_TableInit(&t);

    VTL_tgbot_session* s = VTL_tgbot_session_GetOrCreate(&t, CHAT);
...
TEST(get_or_create_is_stable_per_chat)
{
    ...
    VTL_tgbot_session* a1 = VTL_tgbot_session_GetOrCreate(&t, "100");
    VTL_tgbot_session* a2 = VTL_tgbot_session_GetOrCreate(&t, "100");
    VTL_tgbot_session* b  = VTL_tgbot_session_GetOrCreate(&t, "200");
...
TEST(parse_platform_known_and_unknown)
{
    ...
    VTL_ASSERT(VTL_tgbot_session_ParsePlatform("tg", &bit) == 1, "tg parses");
    VTL_ASSERT(bit == VTL_CONTENT_PLATFORM_TG, "tg -> TG bit");
    VTL_ASSERT(VTL_tgbot_session_ParsePlatform("reddit", &bit) == 1, "reddit parses");
    VTL_ASSERT(bit == VTL_CONTENT_PLATFORM_R, "reddit -> R bit");
    VTL_ASSERT(VTL_tgbot_session_ParsePlatform("WIKI", &bit) == 1, "case-insensitive");
    VTL_ASSERT(bit == VTL_CONTENT_PLATFORM_W, "wiki -> W bit");
    VTL_ASSERT(VTL_tgbot_session_ParsePlatform("nope", &bit) == 0, "unknown rejected");
}

TEST(parse_format_known_and_unknown)
{
    VTL_publication_marked_text_MarkupType m = VTL_markup_type_kStandartMD;
    VTL_ASSERT(VTL_tgbot_session_ParseFormat("asciidoc", &m) == 1, "asciidoc parses");
    VTL_ASSERT(m == VTL_markup_type_kAsciiDoc, "asciidoc -> enum");
    VTL_ASSERT(VTL_tgbot_session_ParseFormat("mediawiki", &m) == 1, "mediawiki parses");
    VTL_ASSERT(m == VTL_markup_type_kMediaWiki, "mediawiki -> enum");
    VTL_ASSERT(VTL_tgbot_session_ParseFormat("html", &m) == 1, "html parses");
    VTL_ASSERT(m == VTL_markup_type_kHTML, "html -> enum");
    VTL_ASSERT(VTL_tgbot_session_ParseFormat("xml", &m) == 0, "unknown rejected");
}

TEST(describe_platforms_lists_selected)
{
    char out[128];
    VTL_tgbot_session_DescribePlatforms(
    ...
    VTL_tgbot_session_DescribePlatforms(0, out, sizeof(out));
    VTL_ASSERT(strcmp(out, "—") == 0, "empty selection shows dash");
}

TEST(format_name_round_trip)
{
    VTL_ASSERT(strcmp(VTL_tgbot_session_FormatName(VTL_markup_type_kAsciiDoc),
                      "AsciiDoc") == 0, "asciidoc name");
    VTL_ASSERT(strcmp(VTL_tgbot_session_FormatName(VTL_markup_type_kMediaWiki),
