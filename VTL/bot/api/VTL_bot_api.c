...
/* Собирает URL метода Bot API в malloc-нутую строку. Вызывающий free(). */
static char* VTL_bot_api_BuildUrl(const char* token, const char* method,
                                  const char* query)
{
    if (!token || !method) return NULL;
    size_t need = strlen(VTL_BOT_API_BASE) + strlen(token) + strlen(method)
                + (query ? strlen(query) : 0) + 16;
    char* url = (char*)malloc(need);
    if (!url) return NULL;
    snprintf(url, need, "%s/bot%s/%s%s",
             VTL_BOT_API_BASE, token, method, query ? query : "");
    return url;
}
...
int VTL_bot_api_GetMe(const char* token, char* out_username, size_t cap)
{
    ...
    char* url = VTL_bot_api_BuildUrl(token, "getMe", NULL);
    if (!url) return 0;
...
int VTL_bot_api_DeleteWebhook(const char* token)
{
    ...
    char* url = VTL_bot_api_BuildUrl(token, "deleteWebhook", NULL);
    if (!url) return 0;
...
char* VTL_bot_api_GetUpdates(const char* token, long offset, int timeout_s)
{
    ...
    char* url = VTL_bot_api_BuildUrl(token, "getUpdates", query);
    if (!url) return NULL;
                                      ...
                                      const char* text)
{
    ...
    /* Тело запроса строим через parson — корректное экранирование текста. */
    ...
    char* url = VTL_bot_api_BuildUrl(token, "sendMessage", NULL);
